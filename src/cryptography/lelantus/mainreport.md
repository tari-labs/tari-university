...
3. **Verify**: All network participants can take the revealed serial number \( s \) and check that it does not appear in any previous spend transaction. Next, they can homomorphically subtract this serial number from all coins in the pool and then check the validity of the provided one-out-of-N proof against this newly composed set of commitments.
...
If the coin \( C_i\) is not tested for negative values, it is possible to create a new value out of the Lelantus coins.  The new coin \( C_i\) is then stored in the Lelantus set of UTXO's.
...
Firo [[[4]]] is a privacy cryptocurrency. It was previously known by the name Zcoin but was rebranded to Firo as of Oct 2020. Firo activated Lelantus on their main net in Jan 2021 [[[5]]], but had to deactivate it in Feb 2021 due to a security breach. As of writing this, they are yet to activate it again.
...
Beam[[[7]]] is an MW based cryptocurrency. Beam activated a version of Lelantus called Lelantus-MW in June 2020 [[[8]]].
...
Here is a talk Vladislav Gelfer, beam developer, made at the ZKProof community event, Amsterdam 2019.
