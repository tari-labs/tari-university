...
| Guarantee of honest dealer | With the standard MPC protocol implementation, there is no guarantee that the dealer behaves honestly according to the protocol and generates challenges honestly, but this aspect could be improved [as suggested1](../../cryptography/bulletproofs-protocols/MainReport.md#mpc-protocol-security-discussion) in that report. | Although each party independently compute challenges themselves, it isn't clear what the implications of a dishonest dealer would be in practice.  |
