...
A hash table is a data structure which maps keys to values. A hashing function is used to compute keys which are inserted into a table
from which values can later be retrieved. As the name suggests, a distributed hash table (DHT) is a hash table which is distributed across
...
An overlay network is simply a communication network built on top of another network. The internet is an example as it began as an overlay
network on the public switched telephone network.

$\langle key, value \rangle$ pairs are stored on a sub-set of the network, usually by some notion of "closeness" to that key.
...
The need for DHTs arose from early file sharing networks such as Gnutella, Napster, FreeNet and BitTorrent who were able to make use of distributed resources across
...
- FreeNet used a key-based routing, however it was less structured than a DHT and does not guarantee that data can be found.

In 2001, four DHT projects were introduced, namely CAN, Chord, Pastry and Tapestry, which aimed to have a similar lookup efficiency ($O(log(n))$)
of a centralized index while having the benefits of a decentralized network.
...
- Communication need only occur between neighbors which are decided by some algorithm.
...
|                                        | CAN                                     | CHORD                   | Kademlia                                                  | Koord                                    | Pastry                             | Tapestry                  | Viceroy                         |
| -------------------------------------- | --------------------------------------- | ----------------------- | --------------------------------------------------------- | ---------------------------------------- | ---------------------------------- | ------------------------- | ------------------------------- |
...
The popularity of _Kademlia_ over other DHTs is likely due to it's relative simplicity and performance. The rest of this section dives deeper into _Kademlia_.
...
- The node existence algorithm resists certain basic DDoS attacks

#### NodeID

A node selects an $n$-bit ID which is opaque to other nodes on the network. The network design
relies on node IDs being uniformly distributed by some random procedure. A nodes position is
determined by the shortest unique prefix of it's ID, which forms a tree structure with node IDs
as leaves. [[2]] This ID should be reused when the node rejoins the network.
...
itself on the network, it must contact one or more bootstrap nodes. These nodes are not special in any way other than
...
3. It sends a `FIND_NODE` lookup request of it's newly generated node ID.
...
5. The joining node then contacts some of the new nodes it knows about and the process continues iteratively until the joining node
...
routing tables with the nodes ID. [[1]]

#### XOR metric

The Kademlia paper published in 2002 offered the novel idea of using the XOR ($\oplus$) operator
...
The XOR metric implicitly captures a notion of distance in the tree structure above. [[2]]

#### Protocol

Kademlia is a relatively simple protocol consisting of only 4 RPC messages which facilitate 2 independent concerns.
Namely, peer discovery and data storage/retrieval.
...
Notably, there is no `JOIN` message. This is because there is no explicit join in _Kademlia_. Each peer has a chance of being added to a
routing table of another node whenever an RPC message is sent/received between them [[2]]. In this way, the node becomes known to the
network.
...
it knows about. The queried node returns the $k$ closest nodes it knows about and the querying node then proceeds in rounds,
querying closer and closer nodes until it has found the node. In the process, both the querying node and the intermediate nodes
have learnt about each other.

##### Data Storage and Retrieval Procedure

The storage and retrieval procedure ensures that $\langle key, value \rangle$ pairs are reliably stored and able to be retrieved
by participants in the network.

The storage procedure uses the [lookup procedure](#lookup-procedure) to locate the closest nodes to the key, at which point, it
issues a `STORE` RPC message to those nodes. Each node republishes the $\langle key, value \rangle$ pairs to increase the availability
of the data. Depending on the implementation, the data may eventually expire (say 24 hours). Therefore, it may be required
for the original publisher to republish the data before that period expires.
...
where the leaves are buckets which contain a maximum of $k$ nodes, aptly named $k$-buckets.
nodes with some common node ID prefix which is captured by the [XOR metric](#xor-metric).
...
$A$, $B$ and $C$ share the same prefix up to the first 2 MSBs, but $A$, $C$ and $D$ share no prefixed bits
and are therefore further apart. In this example, $A$, $B$ and $C$ would in the same bucket and
...
At this point, the node splits the bucket in two - one for nodes which share the same prefix as itself
...
which are long-lived and highly available.
...
- Reliably joining real-world identifiers (such as an IP address, MAC address etc) to the node identifier, and rejecting a threshold of duplicates
...
Given a network which is partitioned into groups or intervals and in which nodes are positioned uniformly and randomly.
Adversaries may proceed in rounds, continuously rejoin nodes from the least faulty group until control is gained over
one or more groups as described in [Adaptive join-leave attacks](#adaptive-join-leave-attack).

The cuckoo rule is a join rule which moves (cuckoos) nodes in the same group as the joining node to random locations
...
- _Balancing Condition_: The interval $I$ contains at least $O(log(n))$ nodes
...
It is concluded that for a constant fraction of adversarial peers, where $\epsilon < 1 - 1/k$ for any constant $k > 1$ is
sufficient to prevent adaptive join-leave attacks with high probability.
...
| _Figure 1: (Cuckoo rule) Minimum group size <br>needed to tolerate different $\epsilon$ for 100,000 rounds.<br>Groups must be large (i.e., 100s to 1000s of nodes) to guarantee correctness._[[7]] |     | Figure 2: (Cuckoo rule) Number of rounds the system maintained correctness with an average group<br> size of 64 nodes, for varied. Simulation was halted<br> after 100,000 rounds. Failure rates drop dramatically past a certain threshold for different N.[[7]] |

Notably, they show that rounds to failure (i.e. more than $1/3$ of nodes in a given group are adversarial) decreases dramatically
...
while maintaining Byzantine correctness. 'Commensal Cuckoo: Secure Group Partitioning for Large-Scale Services'[[7]] warrants more
investigation but is out of the scope of this report.

## Conclusion

DHTs are a proven solution to distributed storage and discovery. _Kademlia_ in particular has been successfully implemented and
sustained in file-sharing and blockchain networks with participants in the millions. Like every network, it is not without it's
flaws and careful network design is required to mitigate attacks.

Novel research exists which propose schemes for protecting networks against control from adversaries. This research becomes
...
[[8]]: Overlay and P2P networks https://www.cs.helsinki.fi/webfm_send/1339 Date accessed: 2019-04-04

[8]: https://www.cs.helsinki.fi/webfm_send/1339 'Overlay and P2P networks'

[[9]]: Poisoning the Kad Network https://www.net.t-labs.tu-berlin.de/~stefan/icdcn10.pdf Date accessed: 2019-04-04
