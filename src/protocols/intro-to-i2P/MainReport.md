...
Messages that leave one device do not travel directly to the inbound tunnel of the destination devices's intended. Instead, the outbound router queries a distributed network database to get the address of the inbound router. This database is comprised of a custom structured distributed hash table (DHT) based off the Kademlia algorithm, and it contains the router information and destination information. This intermediate database is there to find the other client's inbound tunnels efficiently, and to anonymise the addresses of devices communicating with each other. ref: https://geti2p.net/en/about/intro
