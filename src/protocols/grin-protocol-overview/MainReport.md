...
​	$ output - inputs = kernel\_excess +(part\mspace{3mu}of)kernel\_offset $ 
...
​	$ sum(ouputs) - sum(inputs) = sum(kernel\_excess) + kernel\_offset $
...
4. Bob computes the message $ M= fee |lock\_height $, 

   the Schnorr challenge $ e = SHA256(M|K_r \cdot G + K_s\cdot  G|r_r\cdot G + r_s\cdot G) $ 
...
4. Alice sends back her $ s_s = k_s + e\cdot x_s $ as she normally would, now that she can also compute $ e = SHA256(M|k_s\cdot G+k_r\cdot G) $
